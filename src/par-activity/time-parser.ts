 ...
 */
import {casual, Refiner, Parser, Meridiem, ParsedComponents} from "chrono-node";
...
const combineDateAndTime : Refiner = {
  refine: (_, results) => {
    ...
    resultWithDate.start.imply("minute", resultWithTime.start.date().getMinutes());
    const meridiem = resultWithTime.start.date().getHours() < 12 ? Meridiem.AM : Meridiem.PM
    ...
    resultWithTime.start.imply("weekday", resultWithDate.start.get("weekday") as number);
    resultWithTime.start.imply("day", resultWithDate.start.get("day")  as number);
    resultWithTime.start.imply("month", resultWithDate.start.get("month")  as number);
    resultWithTime.start.imply("year", resultWithDate.start.get("year")  as number);
