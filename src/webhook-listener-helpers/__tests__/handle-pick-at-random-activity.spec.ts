...
import { setPickAtRandomAccountActivityHandler, handlePickAtRandomTweetCreateEvents } from "..";
...
describe("handlePickAtRandomAccountActivity", () => {
  ...
  let handlePickAtRandomAccountActivity: (
    event: IActivity, res: VercelResponse
  ) => Promise<boolean>;

  beforeAll(() => {
    handlePickAtRandomAccountActivity = setPickAtRandomAccountActivityHandler(handlePickAtRandomTweetCreateEvents);
  });
